...
default : 위 case에 걸리지 못한 값 (예외처리)일 때  실행되는 부분
*/
...
// todo. swtich문을 이용한 계산기
// ? 첫번째 숫자와 연산자, 그리고 두번째 숫자를 입력받아 계산해주는 계산기
function calculator(){
    /*
        todo.
        1. 함수 실행 시 첫번째 숫자를 입력받는 프롬프트 출력.
        2. 첫번째 숫자를 입력하면 연산자를 입력받는 프롬프트 출력.(단, %나머지 연산자 제외 = 사칙연산만)
        3. 연사자를 입력하면 두번째 숫자를 받는 입력 프롬프트 출력.
        4. 1,2,3 모두가 입력되면 수식과 답을 console에 출력 (ex> 10+10=20)
    */
   var firstNumb = Number(prompt("첫번째 숫자를 입력하세요."));
   var cal = prompt("연산자를 입력하세요. +,-,/,* 중");
   var secondNumb = Number(prompt("두번째 숫자를 입력하세요."));

   switch (cal){
    case "+":
        alert(firstNumb + cal + secondNumb + " = " + (firstNumb + secondNumb));
        break;
    case "-":
        alert(firstNumb + cal + secondNumb + " = " +  (firstNumb - secondNumb));
        break;
