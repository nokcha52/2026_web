/*
    ...
    * 4. undefined
    * 미정의된(초기화 되지 않은, 기본초기값), 들어오지 않은 자료형. 존재하지 않는다
    ...
    * ex > var data = null;
    ...
    ! 이외 함수(function), 객체(object)- (좀 더 큰 범위. 그래서 객체를 불러오면 모든 정보가 불러와지기 땜에 요즘엔 함수를 사용해서 원하는정보만 추출) 등이 존재.
*/
//case 1. 자신이 가장 좋아하는 숫자를 favorNumber 변수에 담아 자료형을 check해보자.
function testTypeof(){
    var favorNumber = 1;
...
// case 3. 현재 자신이 춥다(true), 춥지 않다(false)를 cold 변수에 담아 자료형을 check.
function check1(){
    var cold = false;
...
function check3(){
    var test;
    console.log("초기화하지 않은 변수의 자료 유형은" + typeof(test) + "입니다.")
}