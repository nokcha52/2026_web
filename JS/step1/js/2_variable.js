/*
	...
	4. 게임 등에서 기록되는 레코드나 데이터(에너지 등)를 저장.
...
//? 변수 만들기 var(선언) 변수 이름 = 담길 값(의무 아님); = 선언된 후 값으로 초기화 -> var(선언) 변수이름; = 선언.
//var = variable
//cli client단 adm admin단
var userName = "mark"; //not defined(""가 없으면 문서 전체에서 그 의미를 찾으려함)
var userAge = 99;
//(전역변수) = 어디서 부르든 가능할때 / 지역변수는 그게 안됨
function printLog(){
...
//case 2. 콤마로 구분하며 선언
var radius2,pi2; // ! 변수를 선언만 한다는 것은 undefined 자료 유형 상태가 된다.
radius2 = 1;
pi2 = 3.14;

// case 3. 변수의 특성 = 동일한 이름의 변수 생성하지 말것. 하나의 변수 흐름을 계속 유지시켜야함.
...
// case 4. 연산자 사용시
var numbValue = 10;
// ? numbValue = numbValue + 20;//? 만약 numbValue에 20을 더한다
//=의 의미: 좌변에 우변을 대입하다
numbValue += 20; //? 복합대입연산자 사용시

//todo. 자신의 나이를 변수에 담고 출력하되 20년 뒤 나이를 연산하여 문장으로 출력해보기
var cliName = '지수';
console.log("나의 이름은 " + cliName + "이다.");

// 현재 나이 문장 출력 - 현재 나이는 [값]세이고,
var cliAge = 29;
console.log("현재 나이는 " + cliAge + "세이고,")

// 20년뒤 나이 출력 = 20년 뒤에는 [연산된 값]세 이다.
cliAge += 20;
console.log("20년 뒤에는 " + cliAge + "세이다.")